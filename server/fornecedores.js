...
function pagamentosValidos(lista, documentoEmpresa, { permitirTipoPersonalizado = false } = {}) {
  return (Array.isArray(lista) ? lista : []).map((p) => {
    const tipo = texto(p?.tipo, 30).toLowerCase();
    const tiposFixos = ['pix', 'transferencia', 'dinheiro', 'boleto'];
    if (!tiposFixos.includes(tipo) && !(permitirTipoPersonalizado && tipo === 'outro')) {
      throw new Error('Escolha uma forma de pagamento válida.');
    }
    // Dinheiro não tem favorecido, banco ou chave para registrar. Guardar
    // campos vazios como se fossem conta bancária seria informação enganosa.
    if (tipo === 'dinheiro') return { tipo, principal: !!p?.principal };
    if (tipo === 'boleto') {
      const diasVencimento = Number.parseInt(p?.diasVencimento, 10);
      if (!Number.isInteger(diasVencimento) || diasVencimento < 1 || diasVencimento > 365) {
        throw new Error('Para boleto informe o prazo de pagamento entre 1 e 365 dias.');
      }
      return { tipo, diasVencimento, principal: !!p?.principal };
    }
    if (tipo === 'outro') {
      const tipoPersonalizado = texto(p?.tipoPersonalizado, 60);
      if (!tipoPersonalizado) throw new Error('Informe o nome da forma de pagamento adicionada.');
      return { tipo, tipoPersonalizado, principal: !!p?.principal };
    }
...
function normalizar(dados, opcoes = {}) {
  ...
  return {
    ...
    contatos, pagamentos: pagamentosValidos(dados?.pagamentos, documento, opcoes), observacao: texto(dados?.observacao, 1000),
  };
}
...
async function criar({ unidade, unidadeNome, dados, por }) {
  ...
  const registro = { id: doc.id, unidade, unidadeNome: unidadeNome || unidade, ...normalizar(dados, { permitirTipoPersonalizado: por?.role === 'master' }),
...
async function atualizar(id, dados, por) {
  ...
  const dadosNormalizados = normalizar(dados, { permitirTipoPersonalizado: por?.role === 'master' });
  await ref.update({ ...dadosNormalizados, atualizadoEm: agora, atualizadoPorNome: texto(por?.nome || por?.username || 'Usuário', 80) });
  return { ...atual, ...dadosNormalizados, atualizadoEm: agora };
}
