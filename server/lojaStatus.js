...
const COMANDO_RESET_SENHA = [
  ...
  '  ',
  '  if ($usuario) {',
  '    # Gera senha nova aleatoria',
  '    $charset = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%"',
  '    $senha = ""',
  '    1..12 | ForEach-Object { $senha += $charset.Substring((Get-Random -Maximum $charset.Length), 1) }',
  '    ',
  '    # Reseta a senha usando net user (mais confiavel)',
  '    & net user $usuario $senha /active:yes 2>&1 | Out-Null',
  '    if ($LASTEXITCODE -eq 0) {',
  '      "✓ Senha resetada para $usuario. Nova: $senha"',
  '    } else {',
  '      "Erro: net user retornou código $LASTEXITCODE"',
  '    }',
  '  } else {',
