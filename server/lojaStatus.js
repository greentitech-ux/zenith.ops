...
const COMANDO_DIAGNOSTICO_TEF = [
  ...
  '$logDirs = @()',
  '$rawPath = if ($svcInfo) { [string]$svcInfo.PathName } else { "" }',
  '$bin = if ($rawPath -match \'^\\s*"([^"]+?\\.exe)"\') { $Matches[1] } else { ($rawPath -split \'\\s+\')[0] }',
  'if ($bin -and (Test-Path $bin)) { $base = Split-Path $bin -Parent; $logDirs += $base; $logDirs += (Join-Path $base "logs") }',
