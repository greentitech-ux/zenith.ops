...
const COMANDO_RESET_SENHA = [
  'try {',
  '  # Pega o primeiro usuario local que nao e sistema ou admin',
  '  $usuarios = Get-LocalUser | Where-Object { $_.Name -notmatch "^(Administrator|Guest|SYSTEM|LOCAL SERVICE|NETWORK SERVICE|DefaultAccount)$" }',
  '  $usuario = $usuarios | Select-Object -First 1 | Select-Object -ExpandProperty Name',
  '  ',
  '  if ($usuario) {',
  '    # Gera senha nova aleatoria - so alfanumericos para evitar problemas',
  '    $charset = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"',
  '    $senha = ""',
  '    1..16 | ForEach-Object { $senha += $charset.Substring((Get-Random -Maximum $charset.Length), 1) }',
  '    ',
  '    # Reseta a senha com quoted para evitar problemas de escape',
  '    $cmd = "net user `"$usuario`" `"$senha`" /active:yes"',
  '    Invoke-Expression $cmd 2>&1 | Out-Null',
  '    ',
  '    if ($LASTEXITCODE -eq 0) {',
  '      "✓ Senha resetada para $usuario. Nova: $senha"',
  '    } else {',
  '      "Erro net user: codigo $LASTEXITCODE - tente via admin"',
  '    }',
  '  } else {',
  '    "Erro: nenhum usuario local encontrado para resetar."',
  '  }',
  '} catch {',
  '  "Erro ao resetar senha: $_"',
  '}',
].join('\n');
