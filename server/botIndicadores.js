...
function montarIndicadores({
  ...
  hoje, dias = DIAS_PADRAO, limiteAlertas = 30, limiteSolicitacoes = 40,
}) {
  ...
  return {
    ...
    porUnidade,
    quebrasRelevantes,
    fechamentos: doPeriodo.map(enxugarFechamento).sort((x, y) => (y.data.localeCompare(x.data) || x.unidadeNome.localeCompare(y.unidadeNome, 'pt-BR'))),
    pedidoSemanal: {
