...
function montarScriptVigia({ codigo, posto, tipo, agentToken, noPulsoPrint }) {
  ...
  const linhasComuns = [
    ...
    '        $superficie.Add_MouseUp({ param($s, $e) if ($s.Tag.marcaAtual) { [void]$s.Tag.marcas.Add($s.Tag.marcaAtual); $s.Tag.marcaAtual = $null; $s.Capture = $false; $s.Invalidate(); return }; if($e.Button -eq [System.Windows.Forms.MouseButtons]::Left){$s.Tag.inicio=$null;$s.Tag.areaInicio=$null;$s.Tag.modo=$null;$s.Capture=$false;$s.Invalidate()} })',
