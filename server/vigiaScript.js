...
function montarScriptVigia({ codigo, posto, tipo, agentToken, noPulsoPrint, windowsAntigo, ehServidor, unidadeNome, maquinaNome }) {
  ...
  const linhasComuns = [
    ...
    '  $chaveRel = "Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Taskband"',
    '  if ($operador -ieq $env:USERNAME) {',
    '    try { Remove-ItemProperty -Path ("HKCU:\\" + $chaveRel) -Name Favorites, FavoritesResolve -Force -ErrorAction SilentlyContinue } catch {}',
    '    try { Get-Process -Name explorer -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue; Start-Sleep -Seconds 2; Start-Process -FilePath "explorer.exe" -ErrorAction Stop; Escrever-Log "Barra de tarefas: pinos reconstruidos (registro zerado, Explorer reiniciado na sessao de $operador)."; return $true }',
    '    catch { Escrever-Log "Barra de tarefas: nao consegui reiniciar o Explorer ($($_.Exception.Message)); os pinos atualizam no proximo logon."; return $false }',
    '  }',
    '  $sid = Sid-DoUsuario $operador',
    '  if (-not $sid) { Escrever-Log "Barra de tarefas: sem SID de $operador; os pinos atualizam no proximo logon."; return $false }',
    '  try { Remove-ItemProperty -Path ("Registry::HKEY_USERS\\" + $sid + "\\" + $chaveRel) -Name Favorites, FavoritesResolve -Force -ErrorAction SilentlyContinue } catch {}',
    '  $nomeT = "NOCZenith_ReabrirExplorer"',
    '  try {',
    '    $acaoT = New-ScheduledTaskAction -Execute "explorer.exe"',
    '    $princT = New-ScheduledTaskPrincipal -UserId $operador -LogonType Interactive',
    '    Register-ScheduledTask -TaskName $nomeT -Action $acaoT -Principal $princT -Force -ErrorAction Stop | Out-Null',
    '    # prova que consegue abrir algo NA SESSAO do operador antes de mexer no shell dele',
    '    Start-ScheduledTask -TaskName $nomeT -ErrorAction Stop',
    '  } catch { Escrever-Log "Barra de tarefas: nao consegui preparar o relancamento do Explorer de $operador ($($_.Exception.Message)); nada foi encerrado, os pinos atualizam no proximo logon."; return $false }',
    '  try {',
    '    Start-Sleep -Seconds 2',
    '    foreach ($p in @(Get-CimInstance Win32_Process -Filter "Name = \'explorer.exe\'" -ErrorAction SilentlyContinue)) { try { $o = Invoke-CimMethod -InputObject $p -MethodName GetOwner -ErrorAction Stop; if ($o -and ($o.User -ieq $operador)) { Stop-Process -Id $p.ProcessId -Force -ErrorAction SilentlyContinue } } catch {} }',
    '    Start-Sleep -Seconds 2',
    '    Start-ScheduledTask -TaskName $nomeT -ErrorAction Stop',
    '    Escrever-Log "Barra de tarefas: pinos reconstruidos (registro de $operador zerado, Explorer relancado na sessao dele)."',
    '    return $true',
    '  } catch { Escrever-Log "Barra de tarefas: relancamento do Explorer de $operador falhou ($($_.Exception.Message))."; return $false }',
    '}',
