...
function montarScriptVigia({ codigo, posto, tipo, agentToken, noPulsoPrint, windowsAntigo, unidadeNome, maquinaNome }) {
  ...
  const linhasComuns = [
    ...
    '          # Carimbo unificado: a identificacao da loja e da maquina fica numa placa',
    '          # centralizada no rodape, sem sobrepor a comunicacao da arte.',
    '          $loja = ([string]$NomeLojaArte).ToUpper()',
    '          $maq = ([string]$NomeMaquinaArte).ToUpper()',
    '          $tamFonteLoja = 26',
    '          $tamFonteMaquina = 46',
    '          $corDestaque = [System.Drawing.ColorTranslator]::FromHtml("#e0a33e")',
    ...
    '            $padY = 14 * $esc',
    '            $gap = 4 * $esc',
    '            $placaW = [Math]::Max($tamLoja.Width, $tamMaq.Width) + (2 * $padX)',
    '            $placaH = $tamLoja.Height + $tamMaq.Height + $gap + (2 * $padY)',
    '            $cx = $img.Width / 2.0',
    '            # Coordenadas de referencia: 1669/1920 no layout vertical e rodape no horizontal.',
    '            if ($vertical) { $topo = $img.Height * (1669.0 / 1920.0) } else { $topo = $img.Height * (880.0 / 1080.0) }',
    '            $margemRodape = 30 * $esc',
    '            $topoMax = $img.Height - $placaH - $margemRodape',
    '            $topo = [Math]::Max(16 * $esc, [Math]::Min($topo, $topoMax))',
    '            $xPlaca = $cx - ($placaW / 2.0)',
    '            $raio = 12 * $esc',
    '            $sombra = Retangulo-RedondoCarimbo ([single]($xPlaca + (3 * $esc))) ([single]($topo + (5 * $esc))) ([single]$placaW) ([single]$placaH) ([single]$raio)',
    '            $brSombra = New-Object System.Drawing.SolidBrush([System.Drawing.Color]::FromArgb(115, 0, 0, 0))',
    '            $g.FillPath($brSombra, $sombra)',
    '            $brSombra.Dispose(); $sombra.Dispose()',
    '            $placa = Retangulo-RedondoCarimbo ([single]$xPlaca) ([single]$topo) ([single]$placaW) ([single]$placaH) ([single]$raio)',
    '            $brPlaca = New-Object System.Drawing.SolidBrush([System.Drawing.Color]::FromArgb(226, 7, 31, 51))',
    '            $canetaPlaca = New-Object System.Drawing.Pen($corDestaque, [single][Math]::Max(1, 2 * $esc))',
    '            $g.FillPath($brPlaca, $placa); $g.DrawPath($canetaPlaca, $placa)',
    '            $brPlaca.Dispose(); $canetaPlaca.Dispose(); $placa.Dispose()',
    '            $xLoja = $cx - ($tamLoja.Width / 2.0)',
    '            $xMaq = $cx - ($tamMaq.Width / 2.0)',
    '            $yLoja = $topo + $padY',
    '            $yMaq = $yLoja + $tamLoja.Height + $gap',
    '            $brLoja = New-Object System.Drawing.SolidBrush($corDestaque)',
    '            $brMaq = New-Object System.Drawing.SolidBrush([System.Drawing.Color]::White)',
    '            $g.DrawString($loja, $fLoja, $brLoja, [single]$xLoja, [single]$yLoja)',
    '            $g.DrawString($maq, $fMaq, $brMaq, [single]$xMaq, [single]$yMaq)',
    '            $brLoja.Dispose(); $brMaq.Dispose()',
