...
function montarScriptVigia({ codigo, posto, tipo, agentToken, noPulsoPrint }) {
  ...
  const linhasComuns = [
    ...
    '# sinais vitais do loop do print (batimento, ativo, Ctrl+Q vistos, capturas) -',
    '# "pronto" sozinho nao distinguia loop parado, tecla que nao chega e cadastro',
    '$CaminhoNoPulsoPrintPulso = Join-Path (Split-Path -Parent $PSCommandPath) "nopulso-print.pulso"',
    ...
    '    param($CaminhoAtivo, $PastaBase, $CaminhoLogPrint, $CaminhoErro, $CaminhoPulso, $CaminhoGatilho)',
    ...
    '        $form.Tag = @{ inicio = $null; area = $null; acao = "salvar" }',
    ...
    '        $instrucoes.AutoSize = $true; $instrucoes.Text = "  Arraste para selecionar · pontos das quinas e dos meios redimensionam · dentro move · Enter salva · Ctrl+C só copia · Esc cancela  "',
    ...
    '        $resultado = $form.Tag.resultado; $acaoPrint = $form.Tag.acao; $form.Dispose(); if (-not $resultado) { return $null }; return @{ area = $resultado; acao = $acaoPrint }',
    '      }',
    '      $atalhoAnterior = $false',
    '      $vistosCtrlQ = 0; $capturasFeitas = 0; $ultimoErroCaptura = ""; $ultimoPulsoEm = [DateTime]::MinValue',
    '      # batimento a cada 5s. O script principal le e conta pro NOC: loop vivo?',
    '      # ativo? quantos Ctrl+Q chegaram? quantas capturas sairam? Sem isso',
    '      # "pronto" nao distinguia tecla que nao chega de janela presa.',
    '      function Pulsar-Print($ativoAgora) { try { "$([DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())|ativo=$(if ($ativoAgora) { 1 } else { 0 })|ctrl+q vistos=$vistosCtrlQ|capturas=$capturasFeitas|$ultimoErroCaptura" | Set-Content -Path $CaminhoPulso -Force } catch {} }',
    '      while ($true) {',
    '        $ativo = $false',
    '        try { $ativo = (Test-Path $CaminhoAtivo) -and ((Get-Content $CaminhoAtivo -First 1 -ErrorAction Stop).Trim() -eq "1") } catch {}',
    '        if (([DateTime]::Now - $ultimoPulsoEm).TotalSeconds -ge 5) { Pulsar-Print $ativo; $ultimoPulsoEm = [DateTime]::Now }',
    ...
    '        $atalho = $ctrl -and $q',
    '        $disparou = $atalho -and -not $atalhoAnterior',
    '        if ($disparou) { $vistosCtrlQ++; Log-Print "Ctrl+Q recebido." }',
    '        if ($gatilho) { Log-Print "Captura pedida pelo NOC." }',
    '        if ($disparou -or $gatilho) {',
    '          try {',
    ...
    '            # $atalhoAnterior ANTES do continue: sem isso, cancelar com o Ctrl+Q',
    '            # ainda pressionado reabria a selecao na volta seguinte',
    '            if (-not $escolhaPrint) { $imagem.Dispose(); $atalhoAnterior = $atalho; Log-Print "Selecao cancelada."; continue }',
    ...
    '          } catch { $ultimoErroCaptura = "erro: " + $_.Exception.Message; Log-Print "Falha ao capturar: $($_.Exception.Message)" }',
    '        }',
    ...
    '    }',
    '  }).AddArgument($CaminhoNoPulsoPrintAtivo).AddArgument($PastaNoPulsoPrint).AddArgument($CaminhoLog).AddArgument($CaminhoNoPulsoPrintErro).AddArgument($CaminhoNoPulsoPrintPulso).AddArgument($CaminhoNoPulsoPrintGatilho)',
    '  try { Remove-Item $CaminhoNoPulsoPrintErro -Force -ErrorAction SilentlyContinue } catch {}',
    '  try { Remove-Item $CaminhoNoPulsoPrintPulso -Force -ErrorAction SilentlyContinue } catch {}',
    ...
    '    $pulso = ""',
    '    try { if (Test-Path $CaminhoNoPulsoPrintPulso) { $pulso = (Get-Content $CaminhoNoPulsoPrintPulso -First 1 -ErrorAction Stop).Trim() } } catch {}',
    '    if (-not $pulso) { return "pronto (loop ainda sem batimento)" }',
    '    $partes = $pulso -split "\\|"',
    '    $idade = 0; try { $idade = [int](([DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds() - [long]$partes[0]) / 1000) } catch {}',
    '    $resumo = (($partes | Select-Object -Skip 1) | Where-Object { $_ }) -join " · "',
    '    # batimento parado com o runspace vivo = preso dentro de ShowDialog (overlay',
    '    # aberto mas invisivel). Derruba pra recriar: a janela presa vai junto.',
    '    if ($idade -gt 45) {',
    '      try { $ps.Stop() } catch {}; try { $ps.Dispose() } catch {}',
    '      try { if ($global:NoPulsoPrintRunspace) { $global:NoPulsoPrintRunspace.Close() } } catch {}',
    '      $global:NoPulsoPrintPowerShell = $null; $global:NoPulsoPrintRunspace = $null',
    '      Escrever-Log "NoPulsoPrint travado ha ${idade}s (loop parado, provavel janela de selecao presa) - derrubado pra recriar."',
    '      return "travado ha ${idade}s (janela de selecao presa?) · $resumo"',
    '    }',
