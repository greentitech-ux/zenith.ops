...
function montarScriptVigia({ codigo, posto, tipo, agentToken, noPulsoPrint }) {
  ...
  const urlPapelDeParede = `${APP_BASE_URL}/api/loja-status/papel-de-parede`;
  ...
  const linhasComuns = [
    ...
    '        foreach ($bt in @($btSeta, $btLinha, $btCaixa)) { $bt.Font = $FonteForma }',
    '        $fio1 = Fio-Print',
    ...
    '        $trocaFerramenta = { param($botao, $qual) $janela = $botao.Parent.Parent; if ($janela.Tag.ferramenta -eq $qual) { $janela.Tag.ferramenta = "selecao" } else { $janela.Tag.ferramenta = $qual }; & $janela.Tag.pintarFerramenta $janela; $janela.Tag.ocioso = [DateTime]::UtcNow }',
    ...
    '        $acoes.Controls.AddRange(@($btSeta,$btLinha,$btCaixa,$fio1,$btAfinar,$lblGrossura,$btEngrossar,$btCor,$fio2,$copiar,$salvar,$fio3,$fechar));$form.Controls.Add($acoes)',
    ...
    '  $destino = Join-Path (Split-Path -Parent $PSCommandPath) "papel-de-parede.jpg"',
    '  $chave = "HKCU:\\Control Panel\\Desktop"',
    '  if ($ligado) {',
    '    try { Invoke-WebRequest -Uri $UrlPapelDeParede -OutFile $destino -TimeoutSec 30 -UseBasicParsing } catch { Escrever-Log "Papel de parede: nao baixou ($($_.Exception.Message))"; return }',
