...
function montarScriptVigia({ codigo, posto, tipo, agentToken, noPulsoPrint, windowsAntigo, ehServidor, unidadeNome, maquinaNome }) {
  ...
  const linhasComuns = [
    ...
    '    if ($Servico) {',
    '      $usuarioInterativo = ""',
    '      try { $usuarioInterativo = [string](Get-CimInstance Win32_ComputerSystem -ErrorAction Stop).UserName } catch {}',
    '      $nomeUsuario = ($usuarioInterativo -split "\\\\")[-1]',
    '      if (-not $nomeUsuario) { Escrever-Log "Inventário visual aguardando: não há usuário logado na máquina."; return $false }',
    ...
    '      if (-not $perfil -or -not (Test-Path -LiteralPath $perfil[0].LocalPath)) { Escrever-Log "Inventário visual aguardando: não localizei o perfil de $nomeUsuario."; return $false }',
    '      $perfilUsuario = $perfil[0].LocalPath',
    '    }',
    ...
    '  if ($Servico -or -not $estacao -or -not [bool]$estacao.ativa -or [string]$estacao.modo -ne "aplicar" -or -not [bool]$estacao.gerenciarBarraTarefas) { return $true }',
    ...
    '  $fontes = @($atuais)',
    '  foreach ($origem in @($env:USERPROFILE + "\\Desktop", $env:PUBLIC + "\\Desktop", $env:APPDATA + "\\Microsoft\\Windows\\Start Menu\\Programs", $env:ProgramData + "\\Microsoft\\Windows\\Start Menu\\Programs")) { if (Test-Path -LiteralPath $origem) { $fontes += @(Get-ChildItem -LiteralPath $origem -Filter "*.lnk" -File -Recurse -ErrorAction SilentlyContinue) } }',
