...
// geometria do carimbo do papel de parede - ver carimboLayout.js e docs/CARIMBO.md
const { CARIMBO_LAYOUT } = require('./carimboLayout');
...
function montarScriptVigia({ codigo, posto, tipo, agentToken, noPulsoPrint, windowsAntigo, unidadeNome, maquinaNome }) {
  ...
  const linhasComuns = [
    ...
    '}',
    '',
    'function Enviar-Telemetria($disco, $dispositivos, $impressoras, $vms) {',
    '  try {',
    '    $corpo = @{}',
    '    if ($disco) { $corpo.disco = $disco }',
    '    if ($disco) { $ram = Medir-Ram; if ($ram) { $corpo.ram = $ram } }',
    ...
    '# Geometria: carimboLayout.js (e docs/CARIMBO.md pra humanos). NENHUMA',
    '# medida solta aqui - tudo vem injetado de la, pra mexer no carimbo ser',
    '# mexer num arquivo so.',
    '#',
    '# DOIS blocos INDEPENDENTES, cada um no seu slot reservado da arte:',
    '#   unidade  - regua ambar + nome da LOJA (topo-direito na horizontal,',
    '#              centralizado na vertical)',
    '#   etiqueta - placa branca com o codigo da maquina, centralizada no card',
    '# Ate a arte de setembro/2026 a etiqueta vinha empilhada embaixo do nome;',
    '# agora ela tem slot proprio e os dois blocos nao se falam mais.',
    '#',
    '# Salva em PNG (JPEG sujava a arte de cor chapada - a "pessima qualidade").',
    '# Fonte: o desenho pede Barlow; como nao existe nas maquinas, cai na lista',
    '# de tentativa do carimboLayout.js (condensada mais proxima do Windows).',
    ...
    '  return $path',
    '}',
    '# LETTER-SPACING. System.Drawing nao tem: mede e desenha caractere a',
    '# caractere somando o extra. Perde kerning, o que em caixa alta com',
    '# tracking largo (0.14em) nao aparece. O formato e o GenericTypographic:',
    '# o padrao adiciona uma folga invisivel em volta do texto, e com ela a',
    '# placa nao fecharia no padding pedido pelo desenho.',
    'function Medir-Carimbo($g, $texto, $fonte, $fmt) {',
    '  return $g.MeasureString($texto, $fonte, (New-Object System.Drawing.SizeF 0, 0), $fmt)',
    '}',
    'function Largura-ComEspacamento($g, $texto, $fonte, $extra, $fmt) {',
    '  $w = 0.0',
    '  foreach ($ch in $texto.ToCharArray()) {',
    '    $w = $w + (Medir-Carimbo $g ([string]$ch) $fonte $fmt).Width + $extra',
    '  }',
    '  if ($texto.Length -gt 0) { $w = $w - $extra }',
    '  return $w',
    '}',
    'function Desenhar-ComEspacamento($g, $texto, $fonte, $pincel, $x, $y, $extra, $fmt) {',
    '  $cx = [single]$x',
    '  foreach ($ch in $texto.ToCharArray()) {',
    '    $s = [string]$ch',
    '    $g.DrawString($s, $fonte, $pincel, $cx, [single]$y, $fmt)',
    '    $cx = $cx + (Medir-Carimbo $g $s $fonte $fmt).Width + $extra',
    '  }',
    '}',
    ...
    '          # tudo escala por $esc: com a proporcao preservada, escalar pela',
    '          # largura equivale a usar porcentagem da altura (2560x1440 etc)',
    `          if ($vertical) { $esc = $img.Width / ${CARIMBO_LAYOUT.vertical.base.largura}.0 } else { $esc = $img.Width / ${CARIMBO_LAYOUT.horizontal.base.largura}.0 }`,
    '          $loja = ([string]$NomeLojaArte).ToUpper()',
    '          $maq = ([string]$NomeMaquinaArte).ToUpper()',
    '          if ($vertical) {',
    `            $fonteLoja = ${CARIMBO_LAYOUT.vertical.unidade.fonte}; $topoLoja = ${CARIMBO_LAYOUT.vertical.unidade.topo}; $gapLoja = ${CARIMBO_LAYOUT.vertical.unidade.gap}; $reguaW = ${CARIMBO_LAYOUT.vertical.unidade.reguaW}; $reguaH = ${CARIMBO_LAYOUT.vertical.unidade.reguaH}; $margemDir = 0`,
    `            $fonteEtiq = ${CARIMBO_LAYOUT.vertical.etiqueta.fonte}; $topoEtiq = ${CARIMBO_LAYOUT.vertical.etiqueta.topo}; $raioEtiq = ${CARIMBO_LAYOUT.vertical.etiqueta.raio}; $padXEtiq = ${CARIMBO_LAYOUT.vertical.etiqueta.padX}; $padYEtiq = ${CARIMBO_LAYOUT.vertical.etiqueta.padY}; $lsEtiq = ${CARIMBO_LAYOUT.vertical.etiqueta.espacamento}; $sombraY = ${CARIMBO_LAYOUT.vertical.etiqueta.sombraY}`,
    '          } else {',
    `            $fonteLoja = ${CARIMBO_LAYOUT.horizontal.unidade.fonte}; $topoLoja = ${CARIMBO_LAYOUT.horizontal.unidade.topo}; $gapLoja = ${CARIMBO_LAYOUT.horizontal.unidade.gap}; $reguaW = ${CARIMBO_LAYOUT.horizontal.unidade.reguaW}; $reguaH = ${CARIMBO_LAYOUT.horizontal.unidade.reguaH}; $margemDir = ${CARIMBO_LAYOUT.horizontal.unidade.margemDireita}`,
    `            $fonteEtiq = ${CARIMBO_LAYOUT.horizontal.etiqueta.fonte}; $topoEtiq = ${CARIMBO_LAYOUT.horizontal.etiqueta.topo}; $raioEtiq = ${CARIMBO_LAYOUT.horizontal.etiqueta.raio}; $padXEtiq = ${CARIMBO_LAYOUT.horizontal.etiqueta.padX}; $padYEtiq = ${CARIMBO_LAYOUT.horizontal.etiqueta.padY}; $lsEtiq = ${CARIMBO_LAYOUT.horizontal.etiqueta.espacamento}; $sombraY = ${CARIMBO_LAYOUT.horizontal.etiqueta.sombraY}`,
    '          }',
    `          $corRegua = [System.Drawing.ColorTranslator]::FromHtml("${CARIMBO_LAYOUT.cores.regua}"); $corNome = [System.Drawing.ColorTranslator]::FromHtml("${CARIMBO_LAYOUT.cores.nomeLoja}"); $corEtiqBg = [System.Drawing.ColorTranslator]::FromHtml("${CARIMBO_LAYOUT.cores.etiquetaFundo}"); $corEtiqTxt = [System.Drawing.ColorTranslator]::FromHtml("${CARIMBO_LAYOUT.cores.etiquetaTexto}")`,
    `          $FamiliasCarimbo = @(${CARIMBO_LAYOUT.familias.condensada.map((f) => '"' + f + '"').join(',')})`,
    '          $fLoja = Nova-FonteCarimbo $FamiliasCarimbo ($fonteLoja * $esc) ([System.Drawing.FontStyle]::Bold)',
    '          $fEtiq = Nova-FonteCarimbo $FamiliasCarimbo ($fonteEtiq * $esc) ([System.Drawing.FontStyle]::Bold)',
    '          try {',
    '            # ---- bloco 1: regua ambar + nome da LOJA ----',
    '            $tamLoja = Medir-Carimbo $g $loja $fLoja $fmt',
    '            $y = $topoLoja * $esc',
    '            if ($vertical) {',
    '              $cx = $img.Width / 2.0',
    '              $xRegua = $cx - ($reguaW * $esc) / 2',
    '              $xLoja = $cx - $tamLoja.Width / 2',
    '            } else {',
    '              $dir = $img.Width - ($margemDir * $esc)',
    '              $xRegua = $dir - ($reguaW * $esc)',
    '              $xLoja = $dir - $tamLoja.Width',
    '            }',
    '            $brRegua = New-Object System.Drawing.SolidBrush($corRegua)',
    '            $g.FillRectangle($brRegua, [single]$xRegua, [single]$y, [single]($reguaW * $esc), [single]([Math]::Max(2, $reguaH * $esc)))',
    '            $brRegua.Dispose()',
    '            $y = $y + [Math]::Max(2, $reguaH * $esc) + ($gapLoja * $esc)',
    '            $brSombra = New-Object System.Drawing.SolidBrush([System.Drawing.Color]::FromArgb(90, 0, 0, 0))',
    '            $brLoja = New-Object System.Drawing.SolidBrush($corNome)',
    '            $desl = [Math]::Max(1, $esc)',
    '            $g.DrawString($loja, $fLoja, $brSombra, [single]($xLoja + $desl), [single]($y + $desl), $fmt)',
    '            $g.DrawString($loja, $fLoja, $brLoja, [single]$xLoja, [single]$y, $fmt)',
    '            $brSombra.Dispose(); $brLoja.Dispose()',
    '            # ---- bloco 2: placa branca da MAQUINA, no slot do card ----',
    '            # centralizada na largura da arte; o topo vem do slot reservado',
    '            $extra = $fonteEtiq * $esc * $lsEtiq',
    '            $wTxt = Largura-ComEspacamento $g $maq $fEtiq $extra $fmt',
    '            $hTxt = (Medir-Carimbo $g $maq $fEtiq $fmt).Height',
    '            $etiqW = $wTxt + 2 * ($padXEtiq * $esc)',
    '            $etiqH = $hTxt + 2 * ($padYEtiq * $esc)',
    '            $xEtiq = ($img.Width - $etiqW) / 2.0',
    '            $yEtiq = $topoEtiq * $esc',
    '            $raio = $raioEtiq * $esc',
    '            # sombra: o desenho pede desfoque (0 10px 26px), que o',
    '            # System.Drawing nao faz. Fica o deslocamento translucido -',
    '            # mais perto do desenho do que nao ter sombra nenhuma.',
    '            $pathS = Retangulo-RedondoCarimbo ([single]$xEtiq) ([single]($yEtiq + $sombraY * $esc)) ([single]$etiqW) ([single]$etiqH) ([single]$raio)',
    '            $brS = New-Object System.Drawing.SolidBrush([System.Drawing.Color]::FromArgb(61, 10, 50, 80))',
    '            $g.FillPath($brS, $pathS)',
    '            $brS.Dispose(); $pathS.Dispose()',
    '            $path = Retangulo-RedondoCarimbo ([single]$xEtiq) ([single]$yEtiq) ([single]$etiqW) ([single]$etiqH) ([single]$raio)',
    '            $brBg = New-Object System.Drawing.SolidBrush($corEtiqBg)',
    '            $g.FillPath($brBg, $path)',
    '            $brBg.Dispose(); $path.Dispose()',
    '            $brTxt = New-Object System.Drawing.SolidBrush($corEtiqTxt)',
    '            Desenhar-ComEspacamento $g $maq $fEtiq $brTxt ($xEtiq + $padXEtiq * $esc) ($yEtiq + $padYEtiq * $esc) $extra $fmt',
    '            $brTxt.Dispose()',
  ...
  const linhasLoopInterno = [
    ...
    '      if ($discoAgora -or $redeAgora -or $impAgora -or ($vmsAgora -ne $null)) { Enviar-Telemetria $discoAgora $redeAgora $impAgora $vmsAgora }',
    '    }',
