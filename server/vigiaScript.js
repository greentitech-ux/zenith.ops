...
function montarScriptVigia({ codigo, posto, tipo, agentToken, noPulsoPrint, windowsAntigo, ehServidor, unidadeNome, maquinaNome }) {
  ...
  const linhasComuns = [
    ...
    '  return $null',
    '}',
    '# OS PINOS DA BARRA VIVEM NO REGISTRO (Taskband\\Favorites), nao na pasta',
    '# User Pinned\\TaskBar. Trocar os .lnk da pasta sem zerar o registro deixava o',
    '# icone antigo desenhado na barra apontando pra um arquivo que ja foi pro',
    '# backup - o clique dava "Nao e possivel abrir este item". O Explorer so',
    '# reconstroi os pinos a partir da pasta quando Favorites/FavoritesResolve',
    '# somem E ele reinicia.',
    '# Reiniciar o Explorer de OUTRA sessao deixaria o operador sem barra. Por',
    '# isso, fora da sessao dele, o relancamento vai por tarefa agendada que roda',
    '# como o usuario interativo - testada UMA vez antes de matar qualquer coisa.',
    '# Se nao der pra preparar, nada e morto e os pinos atualizam no proximo logon.',
    '# A tarefa fica registrada (sem gatilho): apagar tarefa em execucao pode',
    '# encerrar o processo dela, e o -Force sobrescreve na proxima vez.',
    '# PINOS DA BARRA. Eles vivem no registro (Taskband\\Favorites), nao na pasta',
    '# User Pinned\\TaskBar - por isso trocar so os .lnk deixava icone morto.',
    '#',
    '# O QUE NAO FAZEMOS MAIS, E POR QUE: zerar o registro so tem efeito quando o',
    '# Explorer reinicia, e reiniciar o Explorer na loja apagava a tela, abria uma',
    '# janela de pasta e sumia com a barra por segundos - com o caixa aberto. O',
    '# Master viu e pediu pra voltar. Entao apenas limpamos o registro do',
    '# OPERADOR: sem matar nada, os pinos se acertam no proximo logon da maquina.',
    '# Ate la o icone antigo pode continuar aparecendo, e isso e proposital -',
    '# icone a mais incomoda menos que a loja piscar no meio do expediente.',
    'function Reconstruir-BarraTarefas {',
    '  $operador = Usuario-DoConsole; if (-not $operador) { $operador = $env:USERNAME }',
    '  $raiz = Raiz-RegistroDoOperador',
    '  $alvo = if ($raiz) { $raiz + "\\Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Taskband" } else { $null }',
    '  if (-not $alvo) { Escrever-Log "Barra de tarefas: sem SID de $operador; os pinos se acertam no proximo logon."; return $false }',
    '  try { Remove-ItemProperty -Path $alvo -Name Favorites, FavoritesResolve -Force -ErrorAction SilentlyContinue } catch {}',
    '  Escrever-Log "Barra de tarefas: pinos de $operador marcados pra reconstruir - valem no proximo logon (nao reiniciamos o Explorer pra nao piscar a tela da loja)."',
    '  return $true',
    '}',
    ...
    '  $fontes = @($atuais)',
    '  foreach ($origem in @(@(Pastas-AreaDeTrabalho $perfilUsuario) + @($perfilUsuario + "\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\Programs", $env:ProgramData + "\\Microsoft\\Windows\\Start Menu\\Programs"))) { if ($origem -and (Test-Path -LiteralPath $origem)) { $fontes += @(Get-ChildItem -LiteralPath $origem -Filter "*.lnk" -File -Recurse -ErrorAction SilentlyContinue) } }',
