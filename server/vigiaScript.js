...
function montarScriptVigia({ codigo, posto, tipo, agentToken, noPulsoPrint }) {
  ...
  const linhasComuns = [
    ...
    '    param($CaminhoAtivo, $PastaBase, $CaminhoLogPrint, $CaminhoErro, $Pulso)',
    ...
    '          Log-Print "Ctrl+Q detectado - abrindo a selecao."',
    ...
    '    }',
    '  }).AddArgument($CaminhoNoPulsoPrintAtivo).AddArgument($PastaNoPulsoPrint).AddArgument($CaminhoLog).AddArgument($CaminhoNoPulsoPrintErro).AddArgument($global:NoPulsoPrintEstado)',
    ...
    '    if ($st.selecaoAbertaEm) { return "selecao aberta ha $([int](((Get-Date) - $st.selecaoAbertaEm)).TotalSeconds)s" }',
    '    if (-not $st.ultimoPulso) { return "iniciando" }',
    '    $paradoHa = [int](((Get-Date) - $st.ultimoPulso)).TotalSeconds',
    '    if ($paradoHa -ge 15) { return "travado ha ${paradoHa}s" }',
    '    return "pronto"',
    '  }',
    '  $motivo = ""',
    ...
    'function Aplicar-NoPulsoPrint($habilitado) {',
    ...
    '    Aplicar-NoPulsoPrint ([bool]$configPrint.noPulsoPrint)',
  ...
  const linhasLoopInterno = [
    ...
    '        try { Aplicar-NoPulsoPrint ([bool]$resp.noPulsoPrint) } catch { Escrever-Log "NoPulsoPrint nao sincronizou: $($_.Exception.Message)" }',
    '      }',
