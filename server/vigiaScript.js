...
function montarScriptVigia({ codigo, posto, tipo, agentToken }) {
  ...
  const linhasLoopInterno = [
    ...
    '    if ($ip) { Invoke-RestMethod -Uri $UrlReportarIp -Method Post -ContentType "application/json" -Headers $CabecalhosAgente -Body (@{ ip = $ip } | ConvertTo-Json) | Out-Null }',
    ...
    '    $contador++',
    '    if ($contador % 6 -eq 0) { Reportar-IpLocal }',
