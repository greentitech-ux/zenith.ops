...
| Autorizações | `autorizacoes.html` | (Master de verdade; sem item no menu — abre pelo push e pelos botões 🔐 Autorizar do NOC e de Usuários) | `qaAprovacoes.js`, `coworkApi.js`, `passkeys.js`, `push.js` | `/api/qa-aprovacoes`, `/api/qa-aprovacoes/:id/aprovar` (digital ou senha), `/api/qa-aprovacoes/:id/rejeitar`, `/api/auth/passkey/confirmar/*` | Autenticada | Pedidos do Claude/Cowork, do Beniboy e do QA Master esperando autorização. Digital primeiro, senha como alternativa. |
